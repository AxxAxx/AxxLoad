...
from qt import *
#dfrom . import __version__
__version__ = '2.1'
from guisave import *
from Axxcom_rc import *
cmdhistory = []
...
class SettingsDialog(QT_QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.settings = QtCore.QSettings('Axxcom', 'Axxcom')
        self.settings.beginGroup("settingsDialog")
        load(self, self.settings)
...
class MainWindow(QT_QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        load_ui_widget(os.path.join(os.path.dirname(__file__), 'Axxcom.ui'), self)
        self.serial = None
    ...
    def eventFilter(self, obj, event):
        if obj is self.input and event.type() == QtCore.QEvent.KeyPress:
            ...
            if event.key() == QtCore.Qt.Key_Down:
                self.call_arrow_down()
        return super().eventFilter(obj, event)
    
    def call_arrow_up(self):
        if self.cmdhistory_pos < len(self.cmdhistory)-1:
            self.cmdhistory_pos += 1
            self.input.setText(self.cmdhistory[self.cmdhistory_pos])
        
    def call_arrow_down(self):
        if self.cmdhistory_pos > 0:
            self.cmdhistory_pos -= 1
            self.input.setText(self.cmdhistory[self.cmdhistory_pos])
    ...
    def on_btn_send(self):
        ...
        try:
            ...
            self.cmdhistory.insert(0,raw.strip().decode('utf-8'))
            self.cmdhistory_pos = -1
            if not USE_THREAD:
    ...
    def recv(self, text):
        ...
        if len(text):
            ...
            self.do_log(text)
            try:
                if text.decode("utf-8").split(" ")[0] == ">>":
                    #print(text.decode("utf-8").split())
                    self.heatsink_temp_label.setText(text.decode("utf-8").split()[2])
                    self.mosfet1_temp_label.setText(text.decode("utf-8").split()[3])
                    self.mosfet2_temp_label.setText(text.decode("utf-8").split()[4])
                    self.pcb_temp_label.setText(text.decode("utf-8").split()[5])
                    
                    self.setcurrent_label.setText(text.decode("utf-8").split()[6])
                    self.measuredcurrent_label.setText(text.decode("utf-8").split()[7])
                    
                    self.measuredvoltage_label.setText(text.decode("utf-8").split()[8])
                    self.measuredpower_label.setText(text.decode("utf-8").split()[9])
                    self.measured_eq_resistance_label.setText(text.decode("utf-8").split()[10])
                    self.discharged_ah_label.setText(text.decode("utf-8").split()[11])
                    self.discharged_wh_label.setText(text.decode("utf-8").split()[12])
                    self.fanspeed_label.setText(text.decode("utf-8").split()[13])
                    
                    
                    
            except:
                print("An exception occurred on Serial parse")
    ...
    def on_about(self):
        ...
        msg.setText("AxxCom " + __version__)
        msg.setInformativeText("Copyright (c) 2020 Axel Johansson")
        msg.setWindowTitle("AxxCom - Axel Johansson 2020")
...
def main():
    ...
    app.setApplicationName("AxxCom")
    win = MainWindow(None)
    win.setWindowTitle("AxxCom " + __version__)
    win.show()
    sys.exit(app.exec_())
